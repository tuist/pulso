...
Early scaffolding. See [`docs/architecture.md`](./docs/architecture.md) for the design; the codebase is still catching up to it.
...
- **Object storage is the source of truth.** S3 (or R2, GCS, Azure Blob, MinIO) holds every acknowledged record. Local disk is a warm cache and nothing more.
...
- **BEAM for orchestration, Rust for bytes.** Elixir/OTP owns concurrency, supervision, backpressure, and the MCP surface. Rust owns Parquet, DataFusion, and the S3 client, called via Rustler NIFs.
